<template>
  <div class="footer">
    <div class="section-one">
      <div height="80px" class="sc-a032055a-0 jnBSko">
        <picture
          ><img
            crossorigin="anonymous"
            src="https://cms-images.mmst.eu/wgppmdcw3act/6faY1YqcuxhxtzmjbaTLSz/21cab873954a5e4a2a50782d68085705/eyebrand-re5.svg?q=80"
            alt="Blindlook"
            height="80px"
            loading="eager"
            fetchpriority="auto"
        /></picture>
      </div>
      <div height="80px" class="sc-a032055a-0 jnBSko">
        <picture
          ><img
            crossorigin="anonymous"
            src="https://cms-images.mmst.eu/wgppmdcw3act/25mxiViFW4wPDdPFsVs9EH/fd7f93a6884c89a648f23bdc4d0d3bb8/daha-iyiye.svg?q=80"
            alt="Daha iyiye"
            height="80px"
            loading="eager"
            fetchpriority="auto"
        /></picture>
      </div>
    </div>
    <div class="section-two">
      <div class="nav-container">
        <span class="header">İletişim</span>
        <div
          :style="{
            display: 'flex',
            justifyContent: 'center',
            alignItems: 'center',
          }"
        >
          <svg width="32" height="32" viewBox="0 0 24 24">
            <path
              fill-rule="evenodd"
              d="M5 7v2H2v7.584l1.2929-1.2911a1 1 0 0 1 .5757-.2842L4 15h7v-3h2v3c0 1.0544-.8159 1.9182-1.8507 1.9945L11 17H4.414l-2.7069 2.7071c-.6026.6026-1.6143.2206-1.7011-.5935L0 19V9c0-1.0544.8159-1.9182 1.8507-1.9945L2 7zm13-7c1.0544 0 1.9182.8159 1.9945 1.8507L20 2v10l-.0017.059-.005.0576-.0089.0598-.0108.0529-.0319.108-.0266.0666-.0708.1322-.064.0893-.0732.0817-.0942.0832-.0768.054-.0963.054-.0359.0168-.0666.0266-.108.0319-.0529.0108-.0598.0089-.0576.005L19 13l-.059-.0017-.0576-.005-.0598-.0089-.0529-.0108-.108-.0319-.0666-.0266-.1322-.0708-.0893-.064-.0817-.0732L15.585 10H9c-1.0544 0-1.9182-.8159-1.9945-1.8507L7 8V2C7 .9456 7.8159.0818 8.8507.0055L9 0zm0 2H9v6h7a1 1 0 0 1 .608.206l.0991.0869L18 9.585z"
            ></path>
          </svg>
          <div
            :style="{
              display: 'flex',
              flexDirection: 'column',
              justifyContent: 'center',
              alignItems: 'start',
              fontSize: 'smaller',
              gap: '5px',
            }"
          >
            <span :style="{ fontSize: 'medium' }">Bize Yazın</span>
            <span :style="{ textDecoration: 'underline', cursor: 'pointer' }"
              >musterihizmetleri@mediamarkt.com.tr</span
            >
          </div>
        </div>
        <div
          :style="{
            display: 'flex',
            justifyContent: 'center',
            alignItems: 'center',
          }"
        >
          <svg width="32" height="32" viewBox="0 0 24 24">
            <path
              fill-rule="evenodd"
              d="M9.5858 0a1 1 0 0 1 .707.2929l5.4143 5.4142A1 1 0 0 1 16 6.4142V18c0 1.1046-.8954 2-2 2H2c-1.1046 0-2-.8954-2-2V2C0 .8954.8954 0 2 0zM8 2H2v16h12V8h-4c-1.0544 0-1.9182-.8159-1.9945-1.8507L8 6zm4 11v2H4v-2zm0-3v2H4v-2zM7 7v2H4V7zm3-4.171V6h3.171z"
            ></path>
          </svg>
          <div
            :style="{
              display: 'flex',
              flexDirection: 'column',
              justifyContent: 'center',
              alignItems: 'start',
              fontSize: 'smaller',
              gap: '5px',
            }"
          >
            <span :style="{ fontSize: 'medium' }">İletişim Formu</span>
            <span :style="{ textDecoration: 'underline', cursor: 'pointer' }"
              >İletişim Formu</span
            >
          </div>
        </div>
        <div
          :style="{
            marginTop: '1rem',
            display: 'flex',
            justifyContent: 'center',
            alignItems: 'center',
            gap: '10px',
          }"
        >
          <span target="_self" aria-label="Social media "
            ><div width="24" height="24">
              <svg width="32" height="32" viewBox="0 0 24 24">
                <path
                  fill-rule="evenodd"
                  d="M12.0005 2C6.4774 2 2 6.4774 2 12.0005c0 4.9915 3.657 9.1287 8.4379 9.8789v-6.9882H7.8987v-2.8907h2.5392V9.7972c0-2.5063 1.493-3.8908 3.7773-3.8908 1.0942 0 2.2386.1954 2.2386.1954v2.461h-1.261c-1.2423 0-1.6298.7709-1.6298 1.5617v1.876h2.7736l-.4434 2.8907H13.563v6.9882c4.7809-.7502 8.4379-4.8874 8.4379-9.8789C22.0009 6.4774 17.5236 2 12.0005 2"
                  clip-rule="evenodd"
                ></path>
              </svg>
            </div> </span
          ><span target="_self" aria-label="Social media "
            ><div width="24" height="24">
              <svg width="32" height="32" viewBox="0 0 24 24">
                <path
                  fill-rule="evenodd"
                  d="M13.0276 2.0008c1.8004.0032 2.1812.0177 3.0954.0594 1.0644.0485 1.7913.2176 2.4274.4648.6576.2555 1.2153.5975 1.7712 1.1534.556.556.8979 1.1136 1.1534 1.7712.2472.636.4163 1.363.4648 2.4274.0105.2286.0192.4238.0265.6162l.0068.1926c.0168.517.024 1.0864.0261 2.2867v2.055c-.0021 1.2003-.0093 1.7697-.0261 2.2867l-.0068.1926c-.0073.1924-.016.3876-.0265.6162-.0485 1.0644-.2176 1.7913-.4648 2.4274-.2555.6576-.5975 1.2152-1.1534 1.7712-.556.556-1.1136.8979-1.7712 1.1534-.6361.2472-1.363.4163-2.4274.4648a79 79 0 0 1-.6162.0264l-.1926.0068c-.517.017-1.0864.024-2.2866.0262h-2.0552c-1.2002-.0022-1.7696-.0093-2.2866-.0262l-.1926-.0068a79 79 0 0 1-.6162-.0264c-1.0644-.0485-1.7913-.2176-2.4274-.4648-.6576-.2555-1.2153-.5975-1.7712-1.1534-.556-.556-.8979-1.1136-1.1534-1.7712-.2472-.636-.4163-1.363-.4648-2.4274-.0418-.9142-.0562-1.295-.0594-3.0954v-2.0551c.0032-1.8005.0176-2.1813.0594-3.0955.0485-1.0644.2176-1.7913.4648-2.4274.2555-.6576.5975-1.2152 1.1534-1.7712.556-.556 1.1136-.8979 1.7712-1.1534.6361-.2472 1.363-.4163 2.4274-.4648.9142-.0417 1.295-.0562 3.0954-.0594Zm-.4717 1.8012H11.444l-.2512.0002-.4538.0008c-1.5517.0039-1.9208.018-2.78.0571-.975.0445-1.5045.2074-1.8568.3443-.4668.1814-.7999.3981-1.1498.748-.35.35-.5667.6831-.748 1.1499-.137.3524-.3.8819-.3444 1.8568-.0392.8592-.0532 1.2283-.057 2.78l-.0009.4538-.0002.2512v1.1118l.0002.2512.0008.4538c.0039 1.5517.0179 1.9208.057 2.78.0446.975.2075 1.5044.3444 1.8568.1814.4668.3981.8.748 1.1498.35.35.6831.5667 1.1499.748.3523.137.8819.3 1.8569.3444.898.041 1.2606.0544 2.999.0575l.7546.0008h.5744l.7546-.0008c1.7384-.003 2.101-.0165 2.999-.0575.975-.0445 1.5046-.2074 1.857-.3443.4667-.1814.7998-.3981 1.1497-.748s.5667-.6831.748-1.1499c.137-.3524.3-.8819.3444-1.8568.041-.8983.0544-1.2609.0575-2.9991l.0007-.7546v-.5743l-.0007-.7547c-.003-1.7382-.0165-2.1008-.0575-2.999-.0445-.975-.2074-1.5045-.3443-1.857-.1814-.4667-.3981-.7998-.748-1.1497s-.6831-.5667-1.1499-.748c-.3523-.137-.8819-.3-1.8569-.3444-.8592-.0392-1.2282-.0532-2.78-.057l-.4537-.0009zM12 7c2.7614 0 5 2.2386 5 5s-2.2386 5-5 5-5-2.2386-5-5 2.2386-5 5-5m0 1.7544c-1.7925 0-3.2456 1.453-3.2456 3.2456S10.2075 15.2456 12 15.2456s3.2456-1.453 3.2456-3.2456S13.7925 8.7544 12 8.7544M17.5 5.5c.5523 0 1 .4477 1 1s-.4477 1-1 1-1-.4477-1-1 .4477-1 1-1"
                ></path>
              </svg>
            </div> </span
          ><span target="_self" aria-label="Social media "
            ><img
              src="https://cms-images.mmst.eu/wgppmdcw3act/5Dz4ytUj4yI6FuJihMCft9/ead9837bc4f5768d55349898948ed129/x-logo.svg?q=80"
              alt=""
              width="24"
              height="24" /></span
          ><span target="_self" aria-label="Social media "
            ><div width="24" height="24">
              <svg width="32" height="32" viewBox="0 0 24 24">
                <path
                  fill-rule="evenodd"
                  d="M12.3536 5.0004c1.3407.0073 6.1284.0588 7.4658.4196.861.2315 1.5388.9139 1.7689 1.7803.3763 1.4134.414 4.2088.4177 4.7537v.1867c-.0038.5449-.0414 3.3402-.4177 4.7538-.23.8664-.908 1.5487-1.7689 1.7803-.5944.1603-1.8703.2595-3.2223.321l-.5094.0213c-1.7018.0654-3.4042.0763-3.9244.0781h-.3216c-.5202-.0018-2.2226-.0127-3.9245-.0781l-.5093-.0213c-1.352-.0615-2.628-.1607-3.2224-.321-.8608-.2316-1.5387-.914-1.7688-1.7803-.3763-1.4136-.414-4.2089-.4177-4.7538v-.1867c.0037-.545.0414-3.3403.4177-4.7537.23-.8664.908-1.5488 1.7688-1.7803 1.3374-.3608 6.1252-.4123 7.4659-.4196Zm-2.3974 4.072v5.9499l5.2294-2.9748z"
                ></path>
              </svg>
            </div>
            <span direction="horizontal" class="sc-ed7fa5f6-0 jfuoKt"></span
          ></span>
        </div>
      </div>
      <div class="nav-container">
        <span class="header">Popüler Kategoriler</span>
        <li>
          <span target="_self" aria-label="Cep Telefonları"
            ><span font-family="default" font-size="xs"
              >Cep Telefonları</span
            ></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Akıllı Saatler"
            ><span font-family="default" font-size="xs"
              >Akıllı Saatler</span
            ></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Tabletler"
            ><span font-family="default" font-size="xs">Tabletler</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Laptop"
            ><span font-family="default" font-size="xs">Laptop</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Televizyon"
            ><span font-family="default" font-size="xs">Televizyon</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Robot Süpürgeler"
            ><span font-family="default" font-size="xs"
              >Robot Süpürgeler</span
            ></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Beyaz Eşyalar"
            ><span font-family="default" font-size="xs"
              >Beyaz Eşyalar</span
            ></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Çamaşır Makineleri"
            ><span font-family="default" font-size="xs"
              >Çamaşır Makineleri</span
            ></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Bulaşık Makineleri"
            ><span font-family="default" font-size="xs"
              >Bulaşık Makineleri</span
            ></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Buzdolabı"
            ><span font-family="default" font-size="xs">Buzdolabı</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Dikey Süpürgeler"
            ><span font-family="default" font-size="xs"
              >Dikey Süpürgeler</span
            ></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Kişisel Bakım Ürünleri"
            ><span font-family="default" font-size="xs"
              >Kişisel Bakım Ürünleri</span
            ></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Klimalar"
            ><span font-family="default" font-size="xs">Klimalar</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Filtre Kahve Makineleri"
            ><span font-family="default" font-size="xs"
              >Filtre Kahve Makineleri</span
            ></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Isıtıcılar"
            ><span font-family="default" font-size="xs">Isıtıcılar</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Kulaklıklar"
            ><span font-family="default" font-size="xs">Kulaklıklar</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Bluetooth Kulaklıklar"
            ><span font-family="default" font-size="xs"
              >Bluetooth Kulaklıklar</span
            ></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Scooter"
            ><span font-family="default" font-size="xs">Scooter</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Telefon Kılıfları"
            ><span font-family="default" font-size="xs"
              >Telefon Kılıfları</span
            ></span
          >
        </li>
      </div>
      <div class="nav-container">
        <span class="header">Popüler Ürünler</span>

        <li>
          <span target="_self" aria-label="Apple iPhone 16"
            ><span>Apple iPhone 16</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 16 Plus"
            ><span>Apple iPhone 16 Plus</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 16 Pro"
            ><span>Apple iPhone 16 Pro</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 16 Pro Max"
            ><span>Apple iPhone 16 Pro Max</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 15"
            ><span>Apple iPhone 15</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 15 Pro"
            ><span>Apple iPhone 15 Pro</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 15 Pro Max"
            ><span>Apple iPhone 15 Pro Max</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 15 Plus"
            ><span>Apple iPhone 15 Plus</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 14"
            ><span>Apple iPhone 14</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 14 Plus"
            ><span>Apple iPhone 14 Plus</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 13"
            ><span>Apple iPhone 13</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 12"
            ><span>Apple iPhone 12</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple iPhone 11 "
            ><span>Apple iPhone 11 </span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Apple Airpods"
            ><span>Apple Airpods</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Samsung Cep Telefonları"
            ><span>Samsung Cep Telefonları</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Huawei Akıllı Saatler"
            ><span>Huawei Akıllı Saatler</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="HP Laptop"
            ><span>HP Laptop</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Samsung TV"
            ><span>Samsung TV</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Dyson v15"
            ><span>Dyson v15</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Dyson Airwrap"
            ><span>Dyson Airwrap</span></span
          >
        </li>
      </div>
      <div class="nav-container">
        <span class="header">Popüler Markalar</span>

        <li>
          <span target="_self" aria-label="Apple"><span>Apple</span></span>
        </li>
        <li>
          <span target="_self" aria-label="Samsung"><span>Samsung</span></span>
        </li>
        <li>
          <span target="_self" aria-label="Dyson"><span>Dyson</span></span>
        </li>
        <li>
          <span target="_self" aria-label="HP"><span>HP</span></span>
        </li>
        <li>
          <span target="_self" aria-label="Bosch"><span>Bosch</span></span>
        </li>
        <li>
          <span target="_self" aria-label="Philips"><span>Philips</span></span>
        </li>
        <li>
          <span target="_self" aria-label="Tefal"><span>Tefal</span></span>
        </li>
        <li>
          <span target="_self" aria-label="Vestel"><span>Vestel</span></span>
        </li>
        <li>
          <span target="_self" aria-label="Nespresso"
            ><span>Nespresso</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Tüm Markalar"
            ><span>Tüm Markalar</span></span
          >
        </li>
      </div>
      <div class="nav-container">
        <span class="header">Konsept Sayfalar</span>

        <li>
          <span target="_self" aria-label="Okul Alışverişi"
            ><span>Okul Alışverişi</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Yılbaşı Hediyeleri"
            ><span>Yılbaşı Hediyeleri</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Barista CLUB"
            ><span>Barista CLUB</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Babalar Günü"
            ><span>Babalar Günü</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Karne Günü Hediyeleri"
            ><span>Karne Günü Hediyeleri</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Yenilenmiş Cep Telefonu"
            ><span>Yenilenmiş Cep Telefonu</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Eskiyi Getir Yeniyi Götür"
            ><span>Eskiyi Getir Yeniyi Götür</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Cihaz Kiralama Hizmeti"
            ><span>Cihaz Kiralama Hizmeti</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Gamezone"
            ><span>Gamezone</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Düğün Paketleri"
            ><span>Düğün Paketleri</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Temizlik Dünyası"
            ><span>Temizlik Dünyası</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Anneler Günü"
            ><span>Anneler Günü</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Black Friday"
            ><span>Black Friday</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Siber Pazartesi (Cyber Monday)"
            ><span>Siber Pazartesi (Cyber Monday)</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="11 Kasım İndirimleri"
            ><span>11 Kasım İndirimleri</span></span
          >
        </li>
        <li>
          <span
            rel="noopener noreferrer"
            target="_blank"
            aria-label="Foto Konsept"
            ><span>Foto Konsept</span></span
          >
        </li>
        <li>
          <span target="_self" aria-label="Teknoloji Konsept"
            ><span>Teknoloji Konsept</span></span
          >
        </li>
        <li>
          <span>TV Alırken Dikkat Edilmesi Gerekenler</span>
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/specials/markalarimiz"
            aria-label="Markalarımız "
            ><span>Markalarımız </span></a
          >
        </li>
      </div>
      <div class="nav-container">
        <span class="header">Güvenli Alışveriş</span>

        <li>
          <a
            target="_self"
            href="https://e-sirket.mkk.com.tr/?page=company&amp;company=10323"
            aria-label="Bilgi Toplumu Hizmetleri"
            ><span>Bilgi Toplumu Hizmetleri</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://ebelge.gib.gov.tr/anasayfa.html"
            aria-label="e-Arşiv Fatura Sorgulama"
            ><span>e-Arşiv Fatura Sorgulama</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/specials/yardim"
            aria-label="Yardım"
            ><span>Yardım</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/specials/kurumsal-satis"
            aria-label="Kurumsal Satış"
            ><span>Kurumsal Satış</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/myaccount/guest-order"
            aria-label="Üyeliksiz Alışveriş Takibi"
            ><span>Üyeliksiz Alışveriş Takibi</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://ux.mediamarkt.com.tr/iade/"
            aria-label="Kolay İade"
            ><span>Kolay İade</span></a
          >
        </li>
      </div>
      <div class="nav-container">
        <span class="header">Kişisel Verilerin Korunması ve Gizliliği</span>

        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/legal/kisisel-verilere-dair-aydinlatma-bildirimi"
            aria-label="Kişisel Verilere Dair Aydınlatma"
            ><span>Kişisel Verilere Dair Aydınlatma</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/legal/cerez-aydinlatma-metni"
            aria-label="Çerez Aydınlatma Metni"
            ><span>Çerez Aydınlatma Metni</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/legal/kvk-isleme-politikasi"
            aria-label="KVK İşleme Politikası"
            ><span>KVK İşleme Politikası</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/legal/veri-sahibi-basvuru-formu"
            aria-label="Veri Sahibi Başvuru Formu"
            ><span>Veri Sahibi Başvuru Formu</span></a
          >
        </li>
        <li>
          <a
            rel="noopener noreferrer"
            target="_blank"
            href="https://www.mediamarkt.com.tr/tr/legal/guvelik-acigi-bildirim-programi"
            aria-label="Güvenlik Açığı Bildirim Programı"
            ><span>Güvenlik Açığı Bildirim Programı</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/legal/gizlilik-politikasi"
            aria-label="Gizlilik Politikası"
            ><span>Gizlilik Politikası</span></a
          >
        </li>
      </div>
      <div class="nav-container">
        <span class="header">Kurumsal</span>

        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/about-us/hakkimizda"
            aria-label="Hakkımızda"
            ><span>Hakkımızda</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/specials/daha-iyiye"
            aria-label="Sürdürülebilirlik"
            ><span>Sürdürülebilirlik</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://careers.mediamarktsaturn.com/MediaMarktTR/content/letsgo/?locale=tr_TR"
            aria-label="İnsan Kaynakları"
            ><span>İnsan Kaynakları</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/about-us/genel-mudurluk"
            aria-label="Genel Müdürlük"
            ><span>Genel Müdürlük</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/specials/musteri-hizmetleri"
            aria-label="Müşteri Hizmetleri"
            ><span>Müşteri Hizmetleri</span></a
          >
        </li>
        <li>
          <a
            rel="noopener noreferrer"
            target="_blank"
            href="https://www.mediamarkt.com.tr/tr/myaccount/guest-newsletter"
            aria-label="Bülten Aboneliği"
            ><span>Bülten Aboneliği</span></a
          >
        </li>
        <li>
          <a
            target="_self"
            href="https://www.mmstartupchallenge.com/tr"
            aria-label="Startup Challenge"
            ><span>Startup Challenge</span></a
          >
        </li>
        <li>
          <a
            rel="noopener noreferrer"
            target="_blank"
            href="https://mediatrend.mediamarkt.com.tr/"
            aria-label="Mediatrend"
            ><span>Mediatrend</span></a
          >
        </li>
      </div>
      <div class="nav-container">
        <span class="header">Mağazalarımız</span>

        <li>
          <a
            target="_self"
            href="https://www.mediamarkt.com.tr/tr/store/magazalarimiz"
            aria-label="En Yakın Mağazayı Bulun"
            ><span>En Yakın Mağazayı Bulun</span></a
          >
        </li>
      </div>
    </div>
    <div class="section-three">
      <span class="header"> MediaMartk uygulamasını indirin </span>
      <div class="app-navs">
        <a
          rel="noreferrer"
          target="_blank"
          href="https://apps.apple.com/tr/app/mediamarkt-t%C3%BCrkiye/id908956795"
          aria-label="Apple"
          ><svg
            width="auto"
            height="40"
            viewBox="0 0 119.664 40"
            role="presentation"
          >
            <path
              d="M110.135 0H9.535Q8.986 0 8.44.002q-.457.004-.919.013A13 13 0 0 0 5.517.19a6.7 6.7 0 0 0-1.9.627 6.4 6.4 0 0 0-1.62 1.18A6.3 6.3 0 0 0 .82 3.617 6.6 6.6 0 0 0 .195 5.52a13 13 0 0 0-.179 2.002c-.01.307-.01.615-.015.921V31.56c.005.31.006.61.015.921a13 13 0 0 0 .18 2.002 6.6 6.6 0 0 0 .624 1.905A6.2 6.2 0 0 0 1.998 38a6.3 6.3 0 0 0 1.618 1.179 6.7 6.7 0 0 0 1.901.63 13.5 13.5 0 0 0 2.004.177c.31.007.613.011.919.011.366.002.728.002 1.095.002h100.6c.36 0 .724 0 1.084-.002.304 0 .617-.004.922-.01a13 13 0 0 0 2-.178 6.8 6.8 0 0 0 1.908-.63A6.3 6.3 0 0 0 117.666 38a6.4 6.4 0 0 0 1.182-1.614 6.6 6.6 0 0 0 .619-1.905 13.5 13.5 0 0 0 .185-2.002c.004-.31.004-.61.004-.921.008-.364.008-.725.008-1.094V9.536q.002-.549-.008-1.092.001-.46-.004-.92a13.5 13.5 0 0 0-.185-2.003 6.6 6.6 0 0 0-.62-1.903 6.47 6.47 0 0 0-2.798-2.8 6.8 6.8 0 0 0-1.908-.627 13 13 0 0 0-2-.176c-.305-.005-.618-.011-.922-.013-.36-.002-.725-.002-1.084-.002"
              style="fill: rgb(166, 166, 166)"
            ></path>
            <path
              d="M8.445 39.125c-.305 0-.602-.004-.904-.01a13 13 0 0 1-1.87-.164 5.9 5.9 0 0 1-1.656-.548 5.4 5.4 0 0 1-1.397-1.016 5.3 5.3 0 0 1-1.02-1.397 5.7 5.7 0 0 1-.544-1.657 12.4 12.4 0 0 1-.166-1.875c-.007-.21-.015-.913-.015-.913v-23.1s.009-.692.015-.895a12.4 12.4 0 0 1 .165-1.872 5.8 5.8 0 0 1 .544-1.662 5.4 5.4 0 0 1 1.015-1.398 5.6 5.6 0 0 1 1.402-1.023 5.8 5.8 0 0 1 1.653-.544A12.6 12.6 0 0 1 7.543.887l.902-.012h102.769l.913.013a12.4 12.4 0 0 1 1.858.162 6 6 0 0 1 1.671.548 5.6 5.6 0 0 1 2.415 2.42 5.8 5.8 0 0 1 .535 1.649 13 13 0 0 1 .174 1.887c.003.283.003.588.003.89.008.375.008.732.008 1.092v20.929c0 .363 0 .718-.008 1.075 0 .325 0 .623-.004.93a13 13 0 0 1-.17 1.853 5.7 5.7 0 0 1-.54 1.67 5.5 5.5 0 0 1-1.016 1.386 5.4 5.4 0 0 1-1.4 1.022 5.9 5.9 0 0 1-1.668.55 12.5 12.5 0 0 1-1.869.163c-.293.007-.6.011-.897.011l-1.084.002z"
            ></path>
            <g data-name="<Group>">
              <g data-name="<Group>">
                <path
                  d="M24.769 20.3a4.95 4.95 0 0 1 2.356-4.151 5.07 5.07 0 0 0-3.99-2.158c-1.68-.176-3.308 1.005-4.164 1.005-.872 0-2.19-.988-3.608-.958a5.32 5.32 0 0 0-4.473 2.728c-1.934 3.348-.491 8.269 1.361 10.976.927 1.325 2.01 2.805 3.428 2.753 1.387-.058 1.905-.885 3.58-.885 1.658 0 2.144.885 3.59.852 1.489-.025 2.426-1.332 3.32-2.67a11 11 0 0 0 1.52-3.092 4.78 4.78 0 0 1-2.92-4.4m-2.732-8.09a4.87 4.87 0 0 0 1.115-3.49 4.96 4.96 0 0 0-3.208 1.66 4.64 4.64 0 0 0-1.144 3.36 4.1 4.1 0 0 0 3.237-1.53"
                  data-name="<Path>"
                  style="fill: rgb(255, 255, 255)"
                ></path>
              </g>
              <path
                d="M42.302 27.14H37.57l-1.137 3.356h-2.005l4.484-12.418h2.083l4.483 12.418h-2.039zm-4.243-1.55h3.752l-1.85-5.446h-.051zm17.101.38c0 2.813-1.506 4.62-3.779 4.62a3.07 3.07 0 0 1-2.848-1.583h-.043v4.484h-1.86V21.442h1.8v1.506h.033a3.21 3.21 0 0 1 2.883-1.6c2.298 0 3.813 1.816 3.813 4.622zm-1.91 0c0-1.833-.948-3.038-2.393-3.038-1.42 0-2.375 1.23-2.375 3.038 0 1.824.955 3.046 2.375 3.046 1.445 0 2.393-1.197 2.393-3.046m11.875 0c0 2.813-1.506 4.62-3.779 4.62a3.07 3.07 0 0 1-2.848-1.583h-.043v4.484h-1.859V21.442h1.799v1.506h.034a3.21 3.21 0 0 1 2.883-1.6c2.298 0 3.813 1.816 3.813 4.622m-1.91 0c0-1.833-.948-3.038-2.393-3.038-1.42 0-2.375 1.23-2.375 3.038 0 1.824.955 3.046 2.375 3.046 1.445 0 2.392-1.197 2.392-3.046zm8.495 1.066c.138 1.232 1.334 2.04 2.97 2.04 1.566 0 2.693-.808 2.693-1.919 0-.964-.68-1.54-2.29-1.936l-1.609-.388c-2.28-.55-3.339-1.617-3.339-3.348 0-2.142 1.867-3.614 4.519-3.614 2.624 0 4.423 1.472 4.483 3.614h-1.876c-.112-1.239-1.136-1.987-2.634-1.987s-2.521.757-2.521 1.858c0 .878.654 1.395 2.255 1.79l1.368.336c2.548.603 3.606 1.626 3.606 3.443 0 2.323-1.85 3.778-4.793 3.778-2.754 0-4.614-1.42-4.734-3.667zM83.346 19.3v2.142h1.722v1.472h-1.722v4.991c0 .776.345 1.137 1.102 1.137a6 6 0 0 0 .611-.043v1.463a5 5 0 0 1-1.032.086c-1.833 0-2.548-.689-2.548-2.445v-5.189h-1.316v-1.472h1.316V19.3zm2.719 6.67c0-2.849 1.678-4.639 4.294-4.639 2.625 0 4.295 1.79 4.295 4.639 0 2.856-1.661 4.638-4.295 4.638s-4.294-1.782-4.294-4.638m6.695 0c0-1.954-.895-3.108-2.401-3.108s-2.4 1.162-2.4 3.108c0 1.962.894 3.106 2.4 3.106s2.401-1.144 2.401-3.106m3.426-4.528h1.773v1.541h.043a2.16 2.16 0 0 1 2.177-1.635 3 3 0 0 1 .637.069v1.738a2.6 2.6 0 0 0-.835-.112 1.873 1.873 0 0 0-1.937 2.083v5.37h-1.858zm13.198 6.395c-.25 1.643-1.85 2.771-3.898 2.771-2.634 0-4.269-1.764-4.269-4.595 0-2.84 1.644-4.682 4.19-4.682 2.506 0 4.08 1.72 4.08 4.466v.637h-6.394v.112a2.358 2.358 0 0 0 2.436 2.564 2.05 2.05 0 0 0 2.09-1.273zm-6.282-2.702h4.526a2.177 2.177 0 0 0-2.22-2.298 2.29 2.29 0 0 0-2.306 2.298"
                style="fill: rgb(255, 255, 255)"
              ></path>
            </g>
            <path
              d="M37.826 8.731a2.64 2.64 0 0 1 2.808 2.965c0 1.906-1.03 3.002-2.808 3.002h-2.155V8.73zm-1.228 5.123h1.125a1.876 1.876 0 0 0 1.967-2.146 1.88 1.88 0 0 0-1.967-2.134h-1.125zm5.082-1.41a2.133 2.133 0 1 1 4.248 0 2.134 2.134 0 1 1-4.247 0zm3.334 0c0-.976-.439-1.547-1.208-1.547-.773 0-1.207.571-1.207 1.547 0 .984.434 1.55 1.207 1.55.77 0 1.208-.57 1.208-1.55m6.559 2.254h-.922l-.93-3.317h-.07l-.927 3.317h-.913l-1.242-4.503h.902l.806 3.436h.067l.926-3.436h.852l.926 3.436h.07l.803-3.436h.889zm2.281-4.503h.855v.715h.066a1.35 1.35 0 0 1 1.344-.802 1.465 1.465 0 0 1 1.559 1.675v2.915h-.889v-2.692c0-.724-.314-1.084-.972-1.084a1.033 1.033 0 0 0-1.075 1.141v2.635h-.888zm5.24-1.758h.888v6.26h-.888zm2.124 4.007a2.133 2.133 0 1 1 4.247 0 2.134 2.134 0 1 1-4.247 0m3.333 0c0-.976-.439-1.547-1.208-1.547-.773 0-1.207.571-1.207 1.547 0 .984.434 1.55 1.207 1.55.77 0 1.208-.57 1.208-1.55m1.849.98c0-.81.604-1.278 1.676-1.344l1.22-.07v-.389c0-.475-.315-.744-.922-.744-.497 0-.84.182-.939.5h-.86c.09-.773.818-1.27 1.84-1.27 1.128 0 1.765.563 1.765 1.514v3.077h-.855v-.633h-.07a1.52 1.52 0 0 1-1.353.707 1.36 1.36 0 0 1-1.501-1.348zm2.895-.384v-.377l-1.1.07c-.62.042-.9.253-.9.65 0 .405.351.64.834.64a1.06 1.06 0 0 0 1.166-.983m2.053-.596c0-1.423.732-2.324 1.87-2.324a1.48 1.48 0 0 1 1.38.79h.067V8.437h.888v6.26h-.851v-.71h-.07a1.56 1.56 0 0 1-1.415.785c-1.145 0-1.869-.901-1.869-2.328m.918 0c0 .955.45 1.53 1.203 1.53.75 0 1.212-.583 1.212-1.526 0-.938-.468-1.53-1.212-1.53-.748 0-1.203.58-1.203 1.526m6.964 0a2.133 2.133 0 1 1 4.247 0 2.134 2.134 0 1 1-4.247 0m3.333 0c0-.976-.438-1.547-1.208-1.547-.772 0-1.207.571-1.207 1.547 0 .984.435 1.55 1.207 1.55.77 0 1.208-.57 1.208-1.55m2.107-2.249h.855v.715h.066a1.35 1.35 0 0 1 1.344-.802 1.465 1.465 0 0 1 1.559 1.675v2.915h-.889v-2.692c0-.724-.314-1.084-.972-1.084a1.033 1.033 0 0 0-1.075 1.141v2.635h-.889zm8.845-1.121v1.141h.976v.749h-.976v2.315c0 .472.194.679.637.679a3 3 0 0 0 .339-.021v.74a3 3 0 0 1-.484.046c-.988 0-1.381-.348-1.381-1.216v-2.543h-.715v-.749h.715V9.074zm2.19-.637h.88v2.481h.07a1.39 1.39 0 0 1 1.374-.806 1.483 1.483 0 0 1 1.55 1.679v2.907h-.889V12.01c0-.72-.335-1.084-.963-1.084a1.052 1.052 0 0 0-1.134 1.142v2.63h-.888zm9.056 5.045a1.83 1.83 0 0 1-1.95 1.303 2.045 2.045 0 0 1-2.081-2.325 2.077 2.077 0 0 1 2.076-2.352c1.253 0 2.009.856 2.009 2.27v.31h-3.18v.05a1.19 1.19 0 0 0 1.2 1.29 1.08 1.08 0 0 0 1.07-.546zm-3.126-1.451h2.275a1.086 1.086 0 0 0-1.109-1.167 1.15 1.15 0 0 0-1.166 1.167"
              data-name="<Group>"
              style="fill: rgb(255, 255, 255)"
            ></path></svg
        ></a>
        <a
          rel="noreferrer"
          target="_blank"
          href="https://play.google.com/store/apps/details?id=tr.com.media.markt"
          aria-label="Google"
          ><svg
            width="auto"
            height="40"
            fill="none"
            viewBox="0 0 338 100"
            role="presentation"
          >
            <g clip-path="url(#google-app-store-en_svg__a)">
              <path
                fill="#000"
                d="M325.481 0H12.519C5.605 0 0 5.596 0 12.5v75C0 94.404 5.605 100 12.518 100h312.963C332.395 100 338 94.404 338 87.5v-75C338 5.596 332.395 0 325.481 0"
              ></path>
              <path
                fill="#A6A6A6"
                d="M325.481 2c5.809 0 10.516 4.7 10.516 10.5v75c0 5.8-4.707 10.5-10.516 10.5H12.519c-5.81 0-10.516-4.7-10.516-10.5v-75C2.003 6.7 6.71 2 12.518 2zm0-2H12.519C5.633 0 0 5.625 0 12.5v75C0 94.375 5.633 100 12.518 100h312.963C332.367 100 338 94.375 338 87.5v-75C338 5.625 332.367 0 325.481 0"
              ></path>
              <path
                fill="#fff"
                stroke="#fff"
                stroke-miterlimit="10"
                stroke-width="0.2"
                d="M115.12 31.875c-.826.3-2.428.775-4.331.775-2.128 0-3.906-.55-5.283-1.875-1.227-1.175-1.978-3.05-1.978-5.25 0-4.2 2.904-7.3 7.661-7.3 1.628 0 2.905.35 3.531.65l-.451 1.475c-.751-.325-1.702-.6-3.13-.6-3.43 0-5.683 2.125-5.683 5.675s2.153 5.7 5.458 5.7c1.202 0 2.003-.175 2.429-.375v-4.2h-2.88v-1.475h4.657zm10.516-6h-5.508v5.1h6.134V32.5h-7.937V18.375h7.636V19.9h-5.808v4.45h5.508v1.5zm6.459-5.95h-4.306v-1.55h10.466v1.55h-4.332V32.5h-1.853V19.925zm14.847-1.55V32.5h-1.827V18.375zm6.285 1.55h-4.307v-1.55h10.466v1.55h-4.332V32.5h-1.852V19.925zm25.112 5.375c0 4.85-2.955 7.425-6.56 7.425s-6.359-2.9-6.359-7.15c0-4.475 2.779-7.425 6.559-7.425s6.36 2.95 6.36 7.15zm-10.966.225c0 3.025 1.627 5.725 4.506 5.725s4.532-2.65 4.532-5.875c0-2.8-1.477-5.75-4.507-5.75s-4.531 2.775-4.531 5.875zm13.77 6.975V18.375h2.003l4.532 7.15c1.051 1.65 1.877 3.15 2.528 4.575h.05c-.175-1.9-.2-3.625-.2-5.825v-5.9h1.728V32.5h-1.853l-4.482-7.175c-.976-1.575-1.928-3.175-2.654-4.7h-.075c.1 1.8.151 3.5.151 5.85V32.5z"
              ></path>
              <path
                fill="#fff"
                d="M170.602 54.375c-5.883 0-10.69 4.475-10.69 10.625s4.807 10.625 10.69 10.625c5.884 0 10.691-4.5 10.691-10.625s-4.807-10.625-10.691-10.625m0 17.075c-3.229 0-6.009-2.65-6.009-6.45s2.78-6.45 6.009-6.45 6.009 2.625 6.009 6.45-2.779 6.45-6.009 6.45m-23.309-17.075c-5.884 0-10.691 4.475-10.691 10.625s4.807 10.625 10.691 10.625 10.691-4.5 10.691-10.625-4.807-10.625-10.691-10.625m0 17.075c-3.23 0-6.009-2.65-6.009-6.45s2.779-6.45 6.009-6.45 6.009 2.625 6.009 6.45-2.779 6.45-6.009 6.45m-27.741-13.825v4.5h10.816c-.326 2.525-1.177 4.4-2.454 5.675-1.577 1.575-4.031 3.3-8.337 3.3-6.66 0-11.868-5.35-11.868-12s5.208-12 11.868-12c3.58 0 6.209 1.4 8.137 3.225l3.18-3.175c-2.704-2.575-6.285-4.55-11.342-4.55-9.114 0-16.775 7.4-16.775 16.525s7.661 16.525 16.775 16.525c4.932 0 8.638-1.6 11.542-4.625C134.073 68.05 135 63.85 135 60.475c0-1.05-.075-2.025-.251-2.825h-15.197zm113.443 3.5c-.877-2.375-3.581-6.775-9.114-6.775s-10.04 4.3-10.04 10.625c0 5.95 4.507 10.625 10.566 10.625 4.882 0 7.711-2.975 8.863-4.7l-3.63-2.425c-1.202 1.775-2.855 2.95-5.233 2.95-2.379 0-4.081-1.075-5.158-3.225l14.246-5.875-.475-1.2zm-14.522 3.55c-.125-4.1 3.18-6.2 5.558-6.2 1.853 0 3.43.925 3.956 2.25zm-11.567 10.3h4.682v-31.25h-4.682zm-7.661-18.25h-.15c-1.052-1.25-3.055-2.375-5.609-2.375-5.333 0-10.215 4.675-10.215 10.675s4.882 10.6 10.215 10.6c2.529 0 4.557-1.125 5.609-2.425h.15v1.525c0 4.075-2.178 6.25-5.684 6.25-2.854 0-4.632-2.05-5.358-3.775l-4.081 1.7c1.177 2.825 4.282 6.275 9.439 6.275 5.483 0 10.115-3.225 10.115-11.075V55h-4.431zm-5.358 14.7c-3.23 0-5.934-2.7-5.934-6.4s2.704-6.475 5.934-6.475 5.683 2.75 5.683 6.475-2.503 6.4-5.683 6.4m61.04-27.7h-11.191v31.25h4.681v-11.85h6.535c5.183 0 10.265-3.75 10.265-9.7s-5.082-9.7-10.265-9.7zm.125 15.075h-6.635V48.075h6.635c3.505 0 5.483 2.9 5.483 5.35s-1.978 5.35-5.483 5.35zm28.868-4.5c-3.38 0-6.885 1.5-8.337 4.775l4.156 1.725c.876-1.725 2.529-2.3 4.256-2.3 2.404 0 4.882 1.45 4.907 4.025v.325c-.851-.475-2.654-1.2-4.882-1.2-4.481 0-9.013 2.45-9.013 7.025 0 4.175 3.655 6.875 7.761 6.875 3.13 0 4.883-1.4 5.959-3.05h.15v2.4h4.507V62.925c0-5.55-4.156-8.65-9.514-8.65zm-.576 17.125c-1.527 0-3.655-.775-3.655-2.65 0-2.4 2.654-3.325 4.957-3.325 2.053 0 3.03.45 4.256 1.05-.35 2.9-2.854 4.95-5.558 4.95zm26.489-16.45-5.358 13.55h-.15l-5.558-13.55h-5.032l8.337 18.95-4.757 10.525h4.882l12.844-29.475zm-42.087 20h4.682v-31.25h-4.682z"
              ></path>
              <path
                fill="#EA4335"
                d="M51.877 48.55 25.212 76.8c.826 3.075 3.63 5.35 6.96 5.35 1.328 0 2.58-.35 3.656-1l.075-.05 29.994-17.275L51.852 48.55z"
              ></path>
              <path
                fill="#FBBC04"
                d="M78.842 43.75h-.025l-12.945-7.525L51.276 49.2l14.646 14.625L78.817 56.4c2.253-1.225 3.78-3.6 3.78-6.325s-1.502-5.1-3.755-6.3z"
              ></path>
              <path
                fill="#4285F4"
                d="M25.212 23.2c-.15.6-.25 1.2-.25 1.85v49.9c0 .65.075 1.25.25 1.85l27.566-27.525z"
              ></path>
              <path
                fill="#34A853"
                d="m52.077 50 13.795-13.775-29.969-17.35a7.2 7.2 0 0 0-3.73-1.025c-3.33 0-6.135 2.275-6.96 5.325z"
              ></path>
            </g>
            <defs>
              <clipPath id="google-app-store-en_svg__a">
                <path fill="#fff" d="M0 0h338v100H0z"></path>
              </clipPath>
            </defs></svg></a
        ><a
          class="sc-2fa46f1d-0 gfGXiX sc-92d183e-0 bGwwlr"
          data-test="huawei-logo"
          rel="noreferrer"
          target="_blank"
          href="https://appgallery.huawei.com/#/app/C101922125"
          aria-label="Huawei"
          ><svg
            width="auto"
            height="40"
            fill="none"
            viewBox="0 0 332 100"
            role="presentation"
          >
            <g clip-path="url(#huawei-app-store-en_svg__a)">
              <path
                fill="#C0C1C1"
                d="M308.579 0H23.436c-.902 0-1.792 0-2.69.004-.753.004-1.499.02-2.26.031-1.636.04-3.291.14-4.927.433-1.644.29-3.174.781-4.673 1.543a15.8 15.8 0 0 0-3.979 2.896 15.4 15.4 0 0 0-2.896 3.987C1.245 10.389.76 11.927.476 13.57.18 15.199.078 16.846.04 18.494c-.023.753-.023 1.51-.039 2.264v58.093c.012.761.016 1.503.039 2.264.043 1.648.144 3.295.441 4.92.285 1.647.773 3.19 1.535 4.68a15.2 15.2 0 0 0 2.896 3.967 15.4 15.4 0 0 0 3.979 2.897c1.499.769 3.025 1.253 4.673 1.55 1.632.293 3.291.39 4.927.433.761.016 1.507.028 2.26.028.898.003 1.788.003 2.69.003H308.58c.883 0 1.781 0 2.663-.004.75 0 1.519-.007 2.264-.027 1.648-.047 3.299-.14 4.916-.433 1.647-.297 3.178-.781 4.688-1.55a15.4 15.4 0 0 0 3.975-2.897 15.7 15.7 0 0 0 2.904-3.966 16.2 16.2 0 0 0 1.523-4.682c.305-1.628.398-3.275.457-4.919.008-.765.008-1.503.008-2.264.019-.894.019-1.78.019-2.69v-52.72c0-.903 0-1.793-.019-2.683 0-.753 0-1.51-.008-2.264-.059-1.648-.152-3.295-.457-4.92a16.2 16.2 0 0 0-1.523-4.677 15.87 15.87 0 0 0-6.879-6.882 16.7 16.7 0 0 0-4.688-1.543c-1.617-.292-3.268-.394-4.916-.433-.749-.012-1.518-.027-2.264-.031-.882-.004-1.78-.004-2.663-.004z"
              ></path>
              <path
                fill="#000"
                d="M20.758 97.45c-.75 0-1.48-.008-2.221-.027-1.375-.04-3.003-.117-4.595-.402-1.5-.274-2.835-.715-4.072-1.347a13.4 13.4 0 0 1-3.436-2.499 13 13 0 0 1-2.506-3.432c-.637-1.241-1.074-2.572-1.336-4.072-.3-1.655-.378-3.33-.41-4.607-.015-.519-.035-2.244-.035-2.244V20.754s.024-1.698.035-2.198c.032-1.288.11-2.963.406-4.603.262-1.507.7-2.838 1.336-4.083a13.2 13.2 0 0 1 2.494-3.436A13.7 13.7 0 0 1 9.866 3.92a14.3 14.3 0 0 1 4.064-1.335c1.655-.297 3.334-.371 4.61-.403l2.218-.03H311.23l2.245.03c1.261.032 2.94.106 4.568.399 1.48.257 2.819.698 4.107 1.347a13.5 13.5 0 0 1 3.42 2.494c1.007 1 1.847 2.16 2.514 3.451a14.2 14.2 0 0 1 1.316 4.053c.285 1.55.375 3.143.425 4.638.008.695.008 1.445.008 2.186.02.922.02 1.8.02 2.682v52.733c0 .894 0 1.765-.02 2.643 0 .8 0 1.53-.008 2.284-.05 1.448-.14 3.037-.421 4.556-.254 1.507-.687 2.838-1.328 4.103a13.4 13.4 0 0 1-2.494 3.404 13.3 13.3 0 0 1-3.44 2.515c-1.276.648-2.577 1.077-4.099 1.35-1.573.286-3.205.364-4.595.403-.719.015-1.472.027-2.206.027z"
              ></path>
              <path
                fill="#fff"
                d="M123.135 67.314h-11.521l-2.819 7.707h-5.063l11.283-28.785h4.786l11.54 28.785h-5.325zm-10.069-3.912h8.601l-3.08-8.245a63 63 0 0 1-1.253-3.775 110 110 0 0 1-1.211 3.736zm20.559-4.567a95 95 0 0 0-.16-5.243h4.607c.172.886.289 1.795.359 2.72q2.859-3.12 6.555-3.119c1.429 0 2.757.36 3.971 1.082q1.827 1.084 2.951 3.416 1.125 2.335 1.125 5.93c0 3.595-.43 4.642-1.285 6.418-.851 1.777-2.05 3.12-3.584 4.033q-2.302 1.37-5.325 1.37-2.184 0-4.31-.733v8.858l-4.908.418v-25.15zm12.903 10.955q1.5-2.015 1.5-5.892 0-3.594-1.242-5.216c-.828-1.077-1.929-1.62-3.307-1.62q-1.43.001-2.643.676c-.808.449-1.577 1.054-2.303 1.807v11.482a7.5 7.5 0 0 0 1.706.566q.973.208 1.928.207 2.858 0 4.361-2.014zm9.643-10.955a95 95 0 0 0-.16-5.243h4.607c.172.886.289 1.795.359 2.72q2.86-3.12 6.555-3.119c1.429 0 2.757.36 3.971 1.082q1.827 1.084 2.951 3.416 1.125 2.335 1.125 5.93c0 3.595-.43 4.642-1.285 6.418-.851 1.777-2.049 3.12-3.584 4.033q-2.302 1.37-5.325 1.37-2.184 0-4.31-.733v8.858l-4.907.418v-25.15zm12.903 10.955q1.5-2.015 1.5-5.892 0-3.594-1.242-5.216c-.828-1.077-1.929-1.62-3.307-1.62q-1.43.001-2.643.676c-.808.449-1.577 1.054-2.303 1.807v11.482a7.5 7.5 0 0 0 1.706.566q.973.208 1.929.207 2.856 0 4.36-2.014zm15.761 3.872q-3.179-1.756-4.888-5.036c-1.14-2.186-1.706-4.767-1.706-7.746q.001-4.849 1.964-8.233 1.967-3.385 5.356-5.084 3.387-1.698 7.598-1.698a20.3 20.3 0 0 1 8.936 2.046l-1.253 3.775q-3.913-1.867-7.468-1.866-3 0-5.255 1.28-2.254 1.283-3.506 3.764c-.832 1.656-1.253 3.654-1.253 6q-.001 3.08.964 5.463c.64 1.589 1.655 2.846 3.037 3.775q2.073 1.392 5.236 1.39c.952 0 1.909-.078 2.861-.238a15 15 0 0 0 2.682-.695v-5.821h-6.652v-3.815h11.423v12.119a25.5 25.5 0 0 1-5.173 1.768 24.3 24.3 0 0 1-5.395.617c-2.889 0-5.388-.586-7.508-1.757zm35.758 1.363a18.6 18.6 0 0 1-.336-2.842q-1.13 1.353-2.951 2.295-1.815.944-4.104.945-1.848 0-3.369-.726a5.63 5.63 0 0 1-2.413-2.186c-.597-.972-.894-2.163-.894-3.565 0-2.08.75-3.736 2.245-4.977q2.243-1.858 7.488-1.859 1.967.001 3.893.277v-.417c0-1.894-.402-3.23-1.215-4.002-.808-.773-1.963-1.164-3.474-1.164q-1.57 0-3.358.457-1.785.457-3.139 1.093l-.773-3.576q1.312-.596 3.459-1.093a20 20 0 0 1 4.529-.496c1.855 0 3.428.281 4.728.844q1.949.844 3.037 2.9c.731 1.37 1.094 3.217 1.094 5.532v8.523c0 .886.05 2.233.16 4.033h-4.611zm-2.94-3.975q1.529-.73 2.503-1.827V65.25c-1.34-.144-2.429-.219-3.28-.219q-3.04 0-4.201.984-1.161.983-1.163 2.651c0 1.113.281 1.847.843 2.354q.844.767 2.296.765c.968 0 1.979-.245 2.998-.734h.004zm11.685-1.17V44.55l4.884-.399v24.85q0 1.37.469 1.936.467.567 1.479.566.456-.001 1.292-.218l.578 3.517q-.696.282-1.698.445-1.002.17-1.897.168c-3.405 0-5.103-1.846-5.103-5.544zm12.216 0V44.55l4.884-.399v24.85q0 1.37.468 1.936.469.567 1.48.566.456-.001 1.292-.218l.578 3.517q-.697.282-1.698.445-1.002.17-1.898.168-5.104.001-5.102-5.544zm29.042-4.947h-14.023q.34 6.734 6.418 6.734c1.008 0 2.034-.12 3.088-.367a18.6 18.6 0 0 0 3.03-.983l1.074 3.357c-2.398 1.163-5.091 1.75-8.086 1.75q-3.414 0-5.762-1.293a8.3 8.3 0 0 1-3.526-3.717q-1.183-2.422-1.183-5.84c0-3.418.418-4.408 1.254-6.118q1.251-2.565 3.486-3.912c1.487-.902 3.209-1.35 5.153-1.35 1.945 0 3.721.472 5.076 1.42q2.032 1.424 3.018 3.783.983 2.367.983 5.224zm-4.786-3.12q-.137-2.564-1.292-3.845c-1.155-1.281-1.796-1.28-3.081-1.28s-2.28.429-3.107 1.292q-1.241 1.294-1.62 3.834zm8.741-2.842q0-1.77-.16-5.383h4.588c.082.597.156 1.307.23 2.135q.106 1.242.148 2.015.854-1.41 1.679-2.374a6.9 6.9 0 0 1 1.929-1.57q1.103-.607 2.51-.605c.753 0 1.405.078 1.948.239l-.617 4.251a5.5 5.5 0 0 0-1.589-.219q-1.728.002-3.018.883-1.292.883-2.76 2.908v13.766h-4.888v-16.05zm15.816 24.822a12 12 0 0 1-1.769-.425l.894-3.576c.332.093.75.18 1.253.27q.756.128 1.429.128 3.255 0 4.888-4.193l.317-.793-7.906-21.612h5.262l3.975 12.079c.648 2.065 1.066 3.65 1.253 4.747q.556-2.325 1.312-4.65l4.033-12.176h4.946l-7.886 21.71q-1.112 3.08-2.405 4.966-1.293 1.885-3.049 2.8-1.762.912-4.24.913a13.4 13.4 0 0 1-2.303-.188zM103.7 18.24h8.847l-.25 2.136h-6.008v4.138h5.665v2.003h-5.665V31.1h6.328l-.218 2.159h-8.702v-15.02zm14.949 7.355-4.669-7.355h2.948l3.162 5.403 3.33-5.403h2.866l-4.627 7.262 5.009 7.757h-2.986l-3.58-5.91-3.643 5.91h-2.862l5.052-7.668zm9.928-7.355h4.045q2.852-.001 4.365 1.163 1.516 1.16 1.515 3.362 0 1.494-.68 2.608a4.5 4.5 0 0 1-1.893 1.71q-1.214.597-2.792.597-.86 0-1.971-.207v5.79h-2.593V18.24zm6.505 6.707c.554-.5.831-1.21.831-2.12s-.281-1.593-.843-2.003c-.562-.406-1.394-.613-2.487-.613h-1.409v5.334c.652.105 1.147.156 1.495.156 1.058 0 1.866-.25 2.416-.754zm5.985-6.707h2.592v12.8h6.215l-.218 2.222h-8.589zm12.746 14.285a6.25 6.25 0 0 1-2.518-2.65q-.87-1.706-.87-3.968.002-2.375.929-4.153a6.5 6.5 0 0 1 2.678-2.749c1.163-.648 2.538-.968 4.123-.968s2.818.312 3.912.937a6.2 6.2 0 0 1 2.498 2.639q.862 1.702.863 3.943 0 2.42-.937 4.201a6.57 6.57 0 0 1-2.678 2.748q-1.738.966-4.08.965c-1.562 0-2.811-.316-3.912-.945zm6.723-1.843q1.09-.733 1.628-2.01.539-1.282.539-2.932c0-1.65-.16-2.042-.476-2.885a4.3 4.3 0 0 0-1.523-2.019q-1.048-.751-2.698-.753c-1.65-.002-1.944.253-2.678.76a4.64 4.64 0 0 0-1.647 2.043q-.55 1.282-.551 2.826 0 1.629.519 2.917c.344.859.875 1.534 1.585 2.034q1.066.744 2.647.745c1.055 0 1.929-.242 2.655-.73zm7.781-12.442h4.701q2.572.001 3.896 1.027t1.323 2.936q-.001 1.193-.476 2.1c-.32.605-.71 1.097-1.183 1.48a5.2 5.2 0 0 1-1.452.85l4.618 6.63h-3.01l-3.912-5.954h-1.909v5.954h-2.592V18.24zm4.064 7.105c.633 0 1.187-.12 1.671-.367q.724-.368 1.132-1.038.404-.67.402-1.55 0-1.129-.714-1.655-.714-.522-2.085-.523h-1.878v5.133zm9.413-7.105h8.847l-.25 2.136h-6.009v4.138h5.665v2.003h-5.665V31.1h6.329l-.219 2.159h-8.702v-15.02zm18.474 0h2.592v15.019h-2.592v-15.02zm9.448 2.167h-4.658l.231-2.167h11.693l-.242 2.167h-4.428v12.852h-2.592V20.407zm18.517 12.118a6.25 6.25 0 0 1-2.518-2.65q-.872-1.706-.871-3.968c0-1.584.313-2.967.93-4.153a6.5 6.5 0 0 1 2.678-2.749c1.163-.648 2.537-.968 4.123-.968 1.585 0 2.818.312 3.911.937a6.2 6.2 0 0 1 2.499 2.639q.862 1.702.863 3.943 0 2.42-.937 4.201a6.57 6.57 0 0 1-2.678 2.748q-1.738.966-4.08.965c-1.562 0-2.811-.316-3.912-.945zm6.723-1.843q1.09-.733 1.628-2.01.539-1.282.539-2.932c0-1.65-.16-2.042-.476-2.885q-.481-1.265-1.523-2.019-1.048-.751-2.698-.753c-1.65-.002-1.944.253-2.678.76a4.65 4.65 0 0 0-1.648 2.043q-.55 1.282-.55 2.826 0 1.629.519 2.917c.344.859.875 1.534 1.585 2.034q1.066.744 2.647.745c1.054 0 1.929-.242 2.655-.73zm7.781-12.442h2.979l7.425 11.638a55 55 0 0 1-.093-3.06V18.24h2.405v15.019h-2.886l-7.542-11.826q.112 2.013.113 3.569v8.257h-2.405v-15.02z"
              ></path>
              <path
                fill="#C8102E"
                fill-rule="evenodd"
                d="M67.287 18.119H39.58c-13.052 0-17.826 4.775-17.826 17.826v27.707c0 13.052 4.774 17.826 17.826 17.826h27.7c13.05 0 17.833-4.774 17.833-17.826V35.945c0-13.051-4.775-17.826-17.826-17.826"
                clip-rule="evenodd"
              ></path>
              <path
                fill="#fff"
                d="M34.301 49.804h1.68v8.285H34.3v-3.365h-3.794v3.365h-1.683v-8.285h1.683v3.342H34.3v-3.342zm9.468 4.744c0 1.347-.668 2.065-1.882 2.065s-1.894-.742-1.894-2.124v-4.68h-1.682v4.735c0 2.33 1.296 3.666 3.553 3.666s3.587-1.363 3.587-3.736V49.8h-1.678v4.743l-.004.004zm19.427.956-1.882-5.696h-1.37l-1.882 5.696-1.831-5.692h-1.788l2.889 8.28h1.39l1.885-5.438 1.882 5.439h1.402l2.88-8.28h-1.74zm6.59-.976h3.056v-1.507h-3.056v-1.694h4.439v-1.51H68.13v8.272h6.255v-1.51h-4.6v-2.05zm6.602 3.553h1.659v-8.273h-1.66zm-27.614-1.718-.754 1.726h-1.718l3.65-8.28h1.484l3.635 8.28h-1.76l-.746-1.726zm.624-1.495h2.542l-1.273-2.963-1.265 2.963zm4.037-15.608c-5.817 0-10.545-4.732-10.545-10.55h1.492c0 4.994 4.064 9.058 9.057 9.058s9.058-4.064 9.058-9.057h1.491c0 5.817-4.732 10.549-10.549 10.549z"
              ></path>
            </g>
            <defs>
              <clipPath id="huawei-app-store-en_svg__a">
                <path fill="#fff" d="M0 0h332v99.601H0z"></path>
              </clipPath>
            </defs></svg
        ></a>
      </div>
    </div>
  </div>
</template>

<script lang="ts" setup></script>

<style>
.footer {
  width: 100%;
  height: auto;
  background-color: rgb(227, 227, 227);
  padding: 5%;
  display: flex;
  flex-direction: column;
  justify-content: start;
  align-items: center;
  gap: 2rem;
}
.section-one {
  display: flex;
  width: 100%;
  justify-content: start;
  align-items: center;
  gap: 3rem;
}
.section-two {
  display: grid;
  width: 100%;
  grid-template-columns: 1fr 1fr 1fr 1fr;
  grid-template-rows: auto;
  gap: 3rem;
}
.section-three {
  display: flex;
  width: 100%;
  flex-direction: column;
  justify-content: start;
  align-items: start;
  gap: 1rem;
}
.app-navs {
  display: flex;
  width: 100%;
  justify-content: start;
  align-items: start;
  gap: 1rem;
}
.nav-container {
  width: 100%;
  height: auto;
  display: flex;
  flex-direction: column;
  justify-content: start;
  align-items: start;
  gap: 10px;
}
.nav-container > span {
  display: flex;
  justify-content: start;
  align-items: start;
}

.nav-container > li {
  list-style: none;
  color: black;
}
.nav-container > li > a {
  color: black;
  font-size: small;
  text-decoration: none;
}

.nav-container > li > a:hover {
  text-decoration: underline;
}

.header {
  font-size: large;
  font-weight: 600;
}
</style>
